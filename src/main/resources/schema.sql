...
--
-- Host: 127.0.0.1    Database: hr_data
-- ------------------------------------------------------
...
INSERT INTO `departments` (`department_id`, `department_name`, `location_id`) VALUES (1,'Administration',1700),(2,'Marketing',1800),(3,'Purchasing',1700),(4,'Human Resources',2400),(5,'Shipping',1500),(6,'IT',1400),(7,'Public Relations',2700),(8,'Sales',2500),(9,'Executive',1700),(10,'Finance',1700),(11,'Accounting',1700);
...
INSERT INTO `people` (`p_id`, `p_name`, `p_age`, `p_height`) VALUES (1,'Mike',23,177),(2,'John',28,187),(3,'Anna',35,197),(4,'Bob',38,167),(5,'James',43,171),(6,'Sam',53,172),(7,'Lisa',63,173),(8,'Tom',73,174);
...
--
-- Dumping routines for database 'hr_data'
--
...
/*!50001 VIEW `emp_details_view` AS select `E`.`employee_id` AS `EMPLOYEE_ID`,`E`.`first_name` AS `FIRST_NAME`,`E`.`last_name` AS `LAST_NAME`,`E`.`job_id` AS `JOB_ID`,`E`.`manager_id` AS `MANAGER_ID`,`E`.`department_id` AS `DEPARTMENT_ID`,`L`.`location_id` AS `LOCATION_ID`,`C`.`country_id` AS `COUNTRY_ID`,`E`.`salary` AS `SALARY`,`D`.`department_name` AS `DEPARTMENT_NAME`,`J`.`job_title` AS `JOB_TITLE`,`L`.`city` AS `CITY`,`L`.`state_province` AS `STATE_PROVINCE`,`C`.`country_name` AS `COUNTRY_NAME`,`R`.`region_name` AS `REGION_NAME` from (((((`employees` `E` join `departments` `D` on((`E`.`department_id` = `D`.`department_id`))) join `jobs` `J` on((`E`.`job_id` = `J`.`job_id`))) join `locations` `L` on((`D`.`location_id` = `L`.`location_id`))) join `countries` `C` on((`L`.`country_id` = `C`.`country_id`))) join `regions` `R` on((`C`.`region_id` = `R`.`region_id`))) */;
...
-- Dump completed on 2024-08-21 22:53:12
